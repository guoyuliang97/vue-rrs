<template>
    <div>
      <div style="background-color:#fff;padding: 20px;text-align: left">
        <p style="margin-bottom: 30px;font-weight:bold">您未完成订单：</p>
           <div  v-for="(item,index) in orderList" :key="index" style="margin-bottom:50px;">
          <div style="color:#aaaaaa">
            <!-- <b >{{item.create_time}}</b> -->
            <div style="font-size:13px;font-weight:bold">订单号：{{item.order_no}}</div>
          </div>
          <div style="display: flex;justify-content: flex-start;margin:10px 0;">
            ...
            <div style="line-height:45px;font-size:14px;margin-left:20px;position: relative">
              ...
              <div>
                <span class="mainBackColor" @click="goFor(item,index)" style="padding:10px;border-radius: 5px;cursor: pointer"><b>立即付款</b></span>
              </div>
            </div>
          </div>
        ...
        <div v-show="goPay" style="position: fixed;top:0;left:0;right:0;bottom:0;background-color: rgba(0,0,0,.3);z-index:999 ;">
    ...
    export default {
      ...
      methods:{
        ...
        goFor(item,index){
          ...
          this.payPrice = item.total_price
        },
        goPayM(){
          if(this.payFor == 1){
            this.isLoading = true
