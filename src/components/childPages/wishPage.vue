  ...
  <div>
    ...
    <div style="margin:80px auto;width:1080px;">
      ...
      <div style="width:100%;display:flex;flex-wrap:wrap;margin-top: 30px;" v-if="activeList.length">
        <div class="storyL"  v-for="(item,index) in activeList" :key="index">
          ...
          <!-- <div class="active" v-if="item.flag == 1">
            <Detail type="active" :data="item"></Detail>
           
          </div> -->
          <div >
            <div style="position:relative" @click="toStory(item,index)">
              <LoadingImg type="2" style="width:250px;height:280px;border-radius:10px;overflow:hidden" :src="item.domain+item.image_url"></LoadingImg>
              <div class="wonderful_img" style="color:#000;">{{item.flag == 1?'活动':'故事'}}</div>
            </div>
