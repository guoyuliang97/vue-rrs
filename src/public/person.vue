<template>
  <div >
    ...
    <div v-if="type=='3'" style="display:flex;justify-content: flex-start">
      <div ><img :src="imgUrl" width="48" height="48" style="border-radius:50%"></div>
    ...
    <div v-if="type=='1'" style="display:flex;justify-content: flex-start">
      <div v-if="type=='1'"><img :src="imgUrl" width="48" height="48" style="border-radius:50%"></div>
    ...
    <div v-if="type=='9'" style="display:flex;justify-content: space-between;">
      ...
      <div v-show="!isShow" style="line-height:88px;color:#008489">
    ...
    <div v-if="type=='love'" style="display:flex;justify-content: flex-start">
      <div ><img :src="imgUrl" width="40" height="40" style="border-radius:50%"></div>
      <div style="padding:5px 0 0 10px;"><span>{{name}}</span></div>
    ...
    <div v-if="type=='replay'" style="display:flex;justify-content: space-between;font-size:14px;padding:10px 0;" @mouseenter="enter" @mouseleave="leave">
      <div style="display:flex;justify-content: flex-start">
        <div style="display: flex;justify-content: flex-start">
          <p style="color:#008489"><b>{{name}}</b></p>&nbsp;回复给&nbsp;<p style="color:#008489"><b>{{otherName}}</b></p>：
        </div>
        <div style="max-width:430px">{{mess}}</div>
      </div>
      <div style="display: flex;justify-content: flex-start">
        <p v-if="replayChoose && !isOwer" style="cursor: pointer" :style="{color:is_report?'#008489':'#000'}"  @click="openInform">{{is_report? '已举报':'举报'}}·</p>
        <p style="margin: 0 0;cursor: pointer"  @click="talk" v-if="isLogin == true">{{isOwer? '删除':'回复'}}</p>
        <p v-if="replayChoose" >·<i class="iconfont icon-zan" style="width:16px;height:16px;" @click="parise" :style="{'color':is_praise?'#008489':'#000'}"></i>{{parseNum}}</p>
      </div>
    </div>
  </div>
</template>
